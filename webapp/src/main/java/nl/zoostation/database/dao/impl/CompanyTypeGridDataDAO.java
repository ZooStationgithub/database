...
@Repository
public class CompanyTypeGridDataDAO extends SessionAwareDAO implements IGridDataDAO<IdNameGridRow> {

    private static final String SELECT_QUERY =
            "select new nl.zoostation.database.model.grid.IdNameGridRow(c.id, c.name) from CompanyType c";

    private static final String COUNT_QUERY =
            "select count(distinct c.id) from CompanyType c";

    @Autowired
    public CompanyTypeGridDataDAO(SessionFactory sessionFactory) {
