...
@Repository
public class RoleTypeGridDataDAO extends SessionAwareDAO implements IGridDataDAO<IdNameGridRow> {

    private static final String SELECT_QUERY =
            "select new nl.zoostation.database.model.grid.IdNameGridRow(r.id, r.name) from RoleType r";

    private static final String COUNT_QUERY =
            "select count(distinct r.id) from RoleType r";

    @Autowired
    public RoleTypeGridDataDAO(SessionFactory sessionFactory) {
