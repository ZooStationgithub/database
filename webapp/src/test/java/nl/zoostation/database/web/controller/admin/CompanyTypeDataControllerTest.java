 ...
 */
public class CompanyTypeDataControllerTest extends ContextAwareBaseControllerTest {

    private static final long ID = 1L;
    private static final String NAME = "Startup";

    @Autowired
    private IGridDataService<IdNameGridRow> companyTypeGridDataService;

    @Autowired
    private IFormService<CompanyType, Long, IdNameFormObject, SimpleFormWrapper<IdNameFormObject>> companyTypeFormService;

    @After
    public void tearDown() throws Exception {
        Mockito.reset(companyTypeGridDataService, companyTypeFormService);
    }

    @Test
    public void testOpenTab() throws Exception {
        getMockMvc().perform(get("/admin/data/company/tab"))
                .andExpect(view().name("/admin/company/companyTab"));
    }
    ...
    public void testGetGridData() throws Exception {
        ...
        when(companyTypeGridDataService.getGridData(argumentCaptor.capture())).thenReturn(gridViewOutputSpec);

        getMockMvc().perform(
                get("/admin/data/company/grid")
                        .param("draw", "2")
    ...
    public void testOpenForm() throws Exception {
        ...
        when(companyTypeFormService.prepareForm(Optional.of(ID))).thenReturn(formWrapper);

        getMockMvc().perform(get("/admin/data/company/form").param("id", String.valueOf(ID)))
                .andExpect(view().name("/admin/company/companyForm"))
                .andExpect(model().attribute("companyType", is(formObject)));
    }

    @Test
    public void testSaveValidationError() throws Exception {
        getMockMvc().perform(
                post("/admin/data/company")
                        .param("id", String.valueOf(ID))
                        .param("name", ""))
                .andExpect(view().name("/admin/company/companyForm"))
                .andExpect(model().attributeExists("companyType"))
                .andExpect(model().attributeHasFieldErrors("companyType", "name"));
    }
    ...
    public void testSaveSuccess() throws Exception {
        ...
        verify(companyTypeFormService).save(argumentCaptor.capture());
    ...
    public void testDeleteObjectInUse() throws Exception {
        doThrow(DataIntegrityViolationException.class).when(companyTypeFormService).delete(ID);

        getMockMvc().perform(delete("/admin/data/company/" + ID))
    ...
    public void testDelete() throws Exception {
        ...
        verify(companyTypeFormService).delete(ID);
    }
}