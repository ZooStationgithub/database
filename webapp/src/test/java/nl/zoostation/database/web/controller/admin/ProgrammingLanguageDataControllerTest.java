 ...
 */
public class ProgrammingLanguageDataControllerTest extends ContextAwareBaseControllerTest {

    private static final long ID = 1L;
    private static final String NAME = "Java";

    @Autowired
    private IGridDataService<IdNameGridRow> programmingLanguageGridDataService;

    @Autowired
    private IFormService<ProgrammingLanguage, Long, IdNameFormObject, SimpleFormWrapper<IdNameFormObject>> programmingLanguageFormService;

    @After
    public void tearDown() throws Exception {
        Mockito.reset(programmingLanguageGridDataService, programmingLanguageFormService);
    }

    @Test
    public void testOpenTab() throws Exception {
        getMockMvc().perform(get("/admin/data/pl/tab"))
                .andExpect(view().name("/admin/pl/plTab"));
    }
    ...
    public void testGetGridData() throws Exception {
        ...
        when(programmingLanguageGridDataService.getGridData(argumentCaptor.capture())).thenReturn(gridViewOutputSpec);

        getMockMvc().perform(
                get("/admin/data/pl/grid")
                        .param("draw", "2")
    ...
    public void testOpenForm() throws Exception {
        ...
        when(programmingLanguageFormService.prepareForm(Optional.of(ID))).thenReturn(formWrapper);

        getMockMvc().perform(get("/admin/data/pl/form").param("id", String.valueOf(ID)))
                .andExpect(view().name("/admin/pl/plForm"))
                .andExpect(model().attribute("programmingLanguage", is(formObject)));
    }
    ...
    public void testSaveValidationError() throws Exception {
        getMockMvc().perform(
                post("/admin/data/pl")
                        ...
                        .param("name", ""))
                .andExpect(view().name("/admin/pl/plForm"))
                .andExpect(model().attributeExists("programmingLanguage"))
                .andExpect(model().attributeHasFieldErrors("programmingLanguage", "name"));
    }
    ...
    public void testSaveSuccess() throws Exception {
        ...
        verify(programmingLanguageFormService).save(argumentCaptor.capture());
    ...
    public void testDeleteObjectInUse() throws Exception {
        doThrow(DataIntegrityViolationException.class).when(programmingLanguageFormService).delete(ID);

        getMockMvc().perform(delete("/admin/data/pl/" + ID))
    ...
    public void testDelete() throws Exception {
        ...
        verify(programmingLanguageFormService).delete(ID);
    }
}