 ...
 */
public class RankTypeDataControllerTest extends ContextAwareBaseControllerTest {

    private static final long ID = 1L;
    private static final String NAME = "Jr";

    @Autowired
    private IGridDataService<IdNameGridRow> rankTypeGridDataService;

    @Autowired
    private IFormService<RankType, Long, IdNameFormObject, SimpleFormWrapper<IdNameFormObject>> rankTypeFormService;

    @After
    public void tearDown() throws Exception {
        Mockito.reset(rankTypeGridDataService, rankTypeFormService);
    }

    @Test
    public void testOpenTab() throws Exception {
        getMockMvc().perform(get("/admin/data/rank/tab"))
                .andExpect(view().name("/admin/rank/rankTab"));
    }
    ...
    public void testGetGridData() throws Exception {
        ...
        when(rankTypeGridDataService.getGridData(argumentCaptor.capture())).thenReturn(gridViewOutputSpec);

        getMockMvc().perform(
                get("/admin/data/rank/grid")
                        .param("draw", "2")
    ...
    public void testOpenForm() throws Exception {
        ...
        when(rankTypeFormService.prepareForm(Optional.of(ID))).thenReturn(formWrapper);

        getMockMvc().perform(get("/admin/data/rank/form").param("id", String.valueOf(ID)))
                .andExpect(view().name("/admin/rank/rankForm"))
                .andExpect(model().attribute("rankType", is(formObject)));
    }

    @Test
    public void testSaveValidationError() throws Exception {
        getMockMvc().perform(
                post("/admin/data/rank")
                        .param("id", String.valueOf(ID))
                        .param("name", ""))
                .andExpect(view().name("/admin/rank/rankForm"))
                .andExpect(model().attributeExists("rankType"))
                .andExpect(model().attributeHasFieldErrors("rankType", "name"));
    }
    ...
    public void testSaveSuccess() throws Exception {
        ...
        verify(rankTypeFormService).save(argumentCaptor.capture());
    ...
    public void testDeleteObjectInUse() throws Exception {
        doThrow(DataIntegrityViolationException.class).when(rankTypeFormService).delete(ID);

        getMockMvc().perform(delete("/admin/data/rank/" + ID))
    ...
    public void testDelete() throws Exception {
        ...
        verify(rankTypeFormService).delete(ID);
    }
}